...
class SubQCATPlotter:
    def __init__(self, data: pd.DataFrame, x_metric: str | None = None):
        self.data = data
        ...
        required_cols = [self.x_metric, 'distance_from_center', 'spatial_cluster']
    ...
    def scatter_plot(self):
        ...
        formatted_metric = self.x_metric.replace('_', ' ').title()
        plt.title(f"Spatial QC: {formatted_metric} vs. Transcript Spread")
        plt.xlabel(f"Standard 10x Metric: {formatted_metric}")
